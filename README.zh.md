...
- 在右侧栏直接编辑，停止输入后自动保存。
- 点击批注行号或定位图标跳回正文。
- 使用批注上的十字准星按钮，将批注重定位到正文中的当前选区或光标。
...
在 Obsidian 中打开「设置 → 第三方插件 → 浏览」，搜索 “Annotation Sidebar” 并安装启用。

### 手动安装

1. 在仓库根目录执行 `npm install` 和 `npm run build`。
2. 在你的库中创建目录 `<Vault>/.obsidian/plugins/annotation-sidebar/`。
3. 将 `dist/` 中的 `main.js`、`manifest.json` 和 `styles.css` 放入该目录。
4. 重启 Obsidian，进入“设置 → 第三方插件”，启用 “Annotation Sidebar”。

`dist/` 是独立的构建产物目录，每次执行构建都会先清理再重新生成，可直接将整个目录的内容作为插件安装或发布。
...
5. 点击行号或定位图标返回正文；需要重新绑定位置时，先在正文中选择内容或放置光标，再点击批注上的十字准星按钮；点击删除图标后确认即可删除。

左侧功能区的消息图标和命令“批注侧栏：打开批注侧栏”都可以打开面板。侧栏标题栏的加号会在最近使用的 Markdown 编辑器选区或光标处添加批注。

打开“设置 → 批注侧栏 → 在正文中显示批注”后，编辑模式和阅读模式都会在批注锚点结束行的下方显示只读批注块。打开批注侧栏后，也可以直接使用标题栏中的“正文显示”开关快速切换。批注块使用独立的视觉层，不会改变 Markdown 文件内容；点击批注块会回到批注侧栏并定位到对应输入框。
...
每条批注都可以在侧栏中选择 `error`、`warn`、`note` 或 `hint` 类型。默认分别使用红色、黄色、蓝色和绿色；类型颜色可在设置中自定义，并会同步应用到正文高亮、位置标记、正文批注块、阅读模式批注块和侧栏卡片。

类型名和颜色均可在设置中自定义；删除类型时，已有批注会转移到另一种保留类型。
